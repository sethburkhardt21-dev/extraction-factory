...
def decide(metrics: dict, *, version_certifiable: bool) -> tuple[str, list[str]]:
    failures: list[str] = []
    ...
    if not version_certifiable:
        return "BLOCKED_EXTERNAL", ["model_version_binding_not_certifiable"]
...
def build_entries(*, registry: dict, identity: dict, observed_version: str, metrics: dict,
                  results_sha256: str, reference_sha256: str, gold_manifest_sha256: str,
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--provider-command", required=True, help="JSON stdin/stdout semantic provider command")
    ...
    try:
        ...
        version_certifiable = observed_version_is_certifiable(
            str(identity.get("observed_version_policy") or ""), args.observed_version
        )
        provider = JSONCommandProvider(
            args.provider_command,
            provider=str(identity["provider"]), model_alias=str(identity["model_alias"]),
            underlying_family=str(identity["underlying_family"]), observed_version=args.observed_version,
            role="COLD_AUDIT", timeout_seconds=max(1, args.timeout), network_required=False,
        ...
        status, failures = decide(metrics, version_certifiable=version_certifiable)
        ...
        entries = build_entries(
            registry=registry, identity=identity, observed_version=args.observed_version,
            metrics=metrics, results_sha256=measured_results_sha,
        ...
        report = {
            "schema_version": "hermes-cold-audit-certification-1.0",
            ...
            "claim_boundary": "Passing proves strict supported-vs-injected-error discrimination only on this exact governed pilot and immutable model version.",
        }
        Path(args.out).write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
        if args.apply:
            if status != "CERTIFIED_WITH_LIMITS":
                print(json.dumps(report, indent=2)); print("refusing --apply without passing certifiable cold-audit benchmark", file=sys.stderr); return 6
            ...
            registry["claim_boundary"] = (
                ...
                " COLD_AUDIT W4 may be CERTIFIED_WITH_LIMITS only through the strict source-first positive/injected-error challenge benchmark on an immutable model version."
            ).strip()

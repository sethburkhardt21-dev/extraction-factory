...
def decide_blind(metrics: dict) -> tuple[str, list[str]]:
    ...
    failures = []
    for name, (measured, floor) in checks.items():
        if measured is None: failures.append(f"{name}: NOT_MEASURED")
        elif measured + 1e-9 < floor: failures.append(f"{name}: measured {measured:.4f}")
    return ("CERTIFIED_WITH_LIMITS" if not failures else "REJECTED"), failures
...
def expected_w2s1_scope(source_units_path: Path, manifest: dict) -> list[str]:
    ...
    for uid, unit in source_by_id.items():
        ...
        if risk.get("work_class") == "W2" and risk.get("source_class") == "S1": out.append(uid)
    return sorted(out)
...
def recompute_and_verify_score(*, score_path: Path, candidates_path: Path, reference_path: Path,
    ...
    if stored.get("role") != role: raise ValueError(f"score_role_mismatch:{stored.get('role')}!={role}")
    model = str(stored.get("model") or "")
    scope = stored.get("units_in_scope")
    if not model: raise ValueError("score_model_missing")
    if not isinstance(scope, list) or not scope or any(not isinstance(x, str) or not x for x in scope): raise ValueError("score_scope_invalid")
    recomputed = build_score_report(
        candidates_path=candidates_path, reference_path=reference_path, gold_manifest_path=gold_manifest_path,
        source_units_path=source_units_path, registry_path=registry_path, role=role, model=model,
        units_in_scope=set(scope), primary_candidates_path=primary_candidates_path,
    )
    ...
    return recomputed, {
        "score_path": str(Path(score_path).resolve()), "score_sha256": sha256_file(score_path),
        ...
        "source_units_sha256": recomputed["source_units_sha256"], "reference_sha256": recomputed["reference_sha256"],
        "gold_manifest_sha256": recomputed["gold_manifest_sha256"], "registry_sha256": recomputed["registry_sha256"],
...
def verify_score_pair(*, primary_score_path: Path, blind_score_path: Path, primary_candidates_path: Path,
    ...
    if manifest.get("benchmark_version") != BENCHMARK_VERSION: raise ValueError(f"benchmark_version_mismatch:{manifest.get('benchmark_version')}")
    expected_scope = expected_w2s1_scope(source_units_path, manifest)
    if not expected_scope: raise ValueError("expected_W2_S1_scope_empty")
    primary, p_receipt = recompute_and_verify_score(
        score_path=primary_score_path, candidates_path=primary_candidates_path, reference_path=reference_path,
        gold_manifest_path=gold_manifest_path, source_units_path=source_units_path, registry_path=registry_path, role="PRIMARY")
    blind, b_receipt = recompute_and_verify_score(
        score_path=blind_score_path, candidates_path=blind_candidates_path, reference_path=reference_path,
        gold_manifest_path=gold_manifest_path, source_units_path=source_units_path, registry_path=registry_path,
    ...
    if primary.get("units_in_scope") != expected_scope or blind.get("units_in_scope") != expected_scope:
        raise ValueError(f"certification_scope_must_equal_all_W2_S1_units:expected={expected_scope}:primary={primary.get('units_in_scope')}:blind={blind.get('units_in_scope')}")
    for field in ["benchmark_version", "gold_label", "reference_sha256", "source_units_sha256", "gold_manifest_sha256"]:
        if primary.get(field) != blind.get(field): raise ValueError(f"primary_blind_benchmark_input_mismatch:{field}")
...
def auto_paths(primary_score_path: Path, blind_score_path: Path) -> dict[str, Path] | None:
    ...
    try:
        p = json.loads(Path(primary_score_path).read_text(encoding="utf-8")); b = json.loads(Path(blind_score_path).read_text(encoding="utf-8"))
        pp = p.get("input_paths"); bp = b.get("input_paths")
        if not isinstance(pp, dict) or not isinstance(bp, dict): return None
        shared = ["reference", "gold_manifest", "source_units", "registry"]
        if any(not pp.get(k) or pp.get(k) != bp.get(k) for k in shared): return None
        if not pp.get("candidates") or not bp.get("candidates") or bp.get("primary_candidates") != pp.get("candidates"): return None
        return {
            "source_units": Path(pp["source_units"]), "reference": Path(pp["reference"]),
            "gold_manifest": Path(pp["gold_manifest"]), "registry": Path(pp["registry"]),
            "primary_candidates": Path(pp["candidates"]), "blind_candidates": Path(bp["candidates"]),
        }
    except Exception:
        return None
...
def build_entry(score: dict, role: str, status: str, failures: list[str], verification: dict | None) -> dict:
    ...
    return {
        "status": status, "decided_at_epoch": round(time.time(), 1),
        "decision_rule": "CERT-W2-S1" if role == "PRIMARY" else "CERT-BLIND-RECALL",
        "measured_metrics": compact, "threshold_failures": failures, "limits": STANDING_LIMITS,
        "gold_reference_sha256": score["reference_sha256"], "gold_manifest_sha256": score.get("gold_manifest_sha256"),
        "source_units_sha256": score.get("source_units_sha256"), "candidate_file_sha256": score.get("candidate_file_sha256"),
        "gold_label": score["gold_label"], "units_in_scope": score["units_in_scope"],
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--primary-score", required=True); parser.add_argument("--blind-score", required=True)
    ...
    parser.add_argument("--registry", default=str(DEFAULT_REGISTRY)); parser.add_argument("--verify-inputs", action="store_true")
    parser.add_argument("--source-units"); parser.add_argument("--reference"); parser.add_argument("--gold-manifest")
    parser.add_argument("--primary-candidates"); parser.add_argument("--blind-candidates")
    ...
    if any(explicit_values):
        if any(not x for x in explicit_values):
            print("explicit verification requires all benchmark input paths", file=sys.stderr); return 3
        paths = {"source_units": Path(args.source_units), "reference": Path(args.reference), "gold_manifest": Path(args.gold_manifest),
                 "registry": Path(args.registry), "primary_candidates": Path(args.primary_candidates), "blind_candidates": Path(args.blind_candidates)}
    else:
    ...
    if (args.verify_inputs or args.apply) and paths is None:
        print("score reports are not self-describing and no complete verification inputs were supplied", file=sys.stderr); return 3
    ...
    try:
        if must_verify and paths is not None:
            ...
            primary, blind, verification = verify_score_pair(
                ...
                primary_candidates_path=paths["primary_candidates"], blind_candidates_path=paths["blind_candidates"],
                reference_path=paths["reference"], gold_manifest_path=paths["gold_manifest"],
                source_units_path=paths["source_units"], registry_path=paths["registry"])
        else:
            primary = json.loads(Path(args.primary_score).read_text(encoding="utf-8")); blind = json.loads(Path(args.blind_score).read_text(encoding="utf-8"))
            for score in (primary, blind):
                if score.get("benchmark_version") != BENCHMARK_VERSION: raise ValueError(f"benchmark version mismatch: {score.get('benchmark_version')}")
    except (ValueError, KeyError, json.JSONDecodeError, OSError) as exc:
        print(f"certification input verification failed: {exc}", file=sys.stderr); return 4

    if args.apply and verification is None:
        print("refusing --apply without successful input recomputation", file=sys.stderr); return 3
    p_identity = primary.get("scored_identity") or {}; b_identity = blind.get("scored_identity") or {}
    p_provider = str(p_identity.get("provider") or ""); b_provider = str(b_identity.get("provider") or "")
    if not p_provider or not b_provider: print("score reports missing scored provider identity", file=sys.stderr); return 4
    if args.primary_provider and args.primary_provider.upper() != p_provider.upper(): print(f"primary provider assertion mismatch:{args.primary_provider}!={p_provider}", file=sys.stderr); return 4
    ...
    if args.apply:
        ...
        print(f"registry updated: {registry_path}"); print("NOTE: registry is protected production state — rerun tests and certify-build after this change.")
    return 0

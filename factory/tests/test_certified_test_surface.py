...
class CertifiedTestSurfaceTests(unittest.TestCase):
    ...
    def test_checkout_policy_is_certified_and_required(self):
        with tempfile.TemporaryDirectory() as raw:
            root, _ = self._root(Path(raw))
            manifest = build_manifest(root)
            paths = {row["path"] for row in manifest["files"]}
            ...
            with self.assertRaisesRegex(RuntimeError, "certified_checkout_policy_missing"):
                build_manifest(root)

    def test_incomplete_checkout_policy_is_rejected(self):
        with tempfile.TemporaryDirectory() as raw:
            root, _ = self._root(Path(raw))
            (root / ".gitattributes").write_text("*.py text eol=lf\n", encoding="utf-8", newline="\n")
            with self.assertRaisesRegex(RuntimeError, "certified_checkout_policy_incomplete"):
                build_manifest(root)

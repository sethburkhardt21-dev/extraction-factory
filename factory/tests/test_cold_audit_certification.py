...
def _ollama_identity():
    return {
        "provider": "OLLAMA", "model_alias": "auditor:1", "underlying_family": "AUDITOR_FAMILY",
        "empirical_semantic_model": True, "independence_group": "AUDITOR_GROUP",
...
def _passing_results(challenges: list[dict]) -> list[dict]:
    return [{
        "challenge_id": c["challenge_id"], "source_unit_id": c["source_unit_id"],
        "mutation": c.get("mutation", "TEST_MUTATION"), "dimension": c.get("dimension"),
        "expected_supported": c["expected_supported"], "actual_supported": c["expected_supported"],
        "correct": True, "request_sha256": "1" * 64,
...
def _coverage_metrics(challenges: list[dict], gold: list[dict], results: list[dict] | None = None) -> dict:
    ...
    return score_results(rows, coverage_summary(challenges, rows, applicable_dimensions(gold)))
...
class ColdAuditChallengeTests(unittest.TestCase):
    ...
    def test_qualifier_and_relationship_dimensions_are_generated_when_applicable(self):
        ...
        rows = build_challenges(gold, {"U1": unit})
        dims = {r["dimension"] for r in rows}
        self.assertIn(DIM_QUALIFIER, dims); self.assertIn(DIM_RELATIONSHIP_DIRECTION, dims)
        self.assertIn(DIM_QUALIFIER, applicable_dimensions(gold)); self.assertIn(DIM_RELATIONSHIP_DIRECTION, applicable_dimensions(gold))
    ...
    def test_missing_applicable_dimension_blocks_certification(self):
        ...
        metrics = score_results(results, coverage_summary(challenges, results, applicable_dimensions(gold)))
    ...
    def test_one_error_in_one_dimension_blocks_certification(self):
        ...
        challenges = build_challenges(gold, {"U1": unit}); results = _passing_results(challenges)
        target = next(r for r in results if r["dimension"] == DIM_RELATIONSHIP_DIRECTION)
        target["actual_supported"] = True; target["correct"] = False
        status, failures = decide(_coverage_metrics(challenges, gold, results), version_certifiable=True, version_authority_verified=True)
        self.assertEqual(status, "REJECTED"); self.assertTrue(any("RELATIONSHIP_DIRECTION" in x for x in failures))
    ...
    def test_semantic_freshness_ignores_rationale_receipt_and_request_noise(self):
        ...
        changed_noise[0]["request_sha256"] = "9" * 64; changed_noise[0]["verdict"]["rationale"] = "different"
        changed_noise[0]["provider_receipt"]["duration_seconds"] = 99.0
        self.assertEqual(challenge_semantic_sha256(challenges, results), challenge_semantic_sha256(challenges, changed_noise))
        changed_decision = copy.deepcopy(results); changed_decision[1]["actual_supported"] = True; changed_decision[1]["correct"] = False
        self.assertNotEqual(challenge_semantic_sha256(challenges, results), challenge_semantic_sha256(challenges, changed_decision))
...
class ColdAuditGoldIndependenceTests(unittest.TestCase):
    def test_auditor_must_be_disjoint_from_all_gold_construction_groups(self):
        manifest = {"gold_construction_independence_groups": ["GOLD_A", "GOLD_B", "GOLD_C"]}; identity = _ollama_identity()
    ...
    def test_missing_or_non_distinct_gold_lineage_fails_closed(self):
        ...
        with self.assertRaisesRegex(ValueError, "missing_or_incomplete"): validate_gold_construction_disjointness({}, identity)
...
class ColdAuditProviderAuthorityTests(unittest.TestCase):
    def test_ollama_provider_uses_measured_digest_and_guarded_command(self):
        ...
        self.assertEqual(observed, DIGEST); self.assertTrue(verified); self.assertEqual(provider.identity().observed_version, DIGEST)
        joined = " ".join(provider.command); self.assertIn("ollama_digest_guard.py", joined); self.assertIn(DIGEST, joined)
    ...
    def test_ollama_operator_version_assertion_mismatch_fails(self):
        ...
        with patch("benchmarks_ext.cold_audit_certifier.resolve_digest", return_value=DIGEST):
            with self.assertRaisesRegex(ValueError, "assertion_mismatch"): _build_provider(args, _ollama_identity())

    def test_non_ollama_generic_command_remains_descriptive_not_certifiable(self):
        identity = dict(_ollama_identity()); identity.update({"provider": "HOSTED", "model_alias": "hosted-model", "observed_version_policy": "EXPLICIT_IMMUTABLE_VERSION"})
        ...
        self.assertEqual(observed, "provider-version-1"); self.assertFalse(verified); self.assertEqual(provider.identity().observed_version, observed)
...
class ColdAuditRuntimeCertificationTests(unittest.TestCase):
    def _registry_and_entry(self):
        ...
        coverage = {
            ...
            "missing_required_dimensions": [], "all_required_dimensions_covered": True,
            "per_dimension": {
        ...
        entries = build_entries(registry={}, identity=identity, observed_version=DIGEST,
            ...
            source_units_sha256="4" * 64, units_in_scope=["U1"], source_classes=["S1"], status="CERTIFIED_WITH_LIMITS", failures=[])
        key = next(iter(entries))
        registry = {"model_identities": {"OLLAMA|auditor:1": {"underlying_family": "AUDITOR_FAMILY", "empirical_semantic_model": True,
            "independence_group": "AUDITOR_GROUP", "observed_version_policy": "OLLAMA_DIGEST"}}, "certifications": entries, "retired_certification_keys": []}
        return registry, key

    def test_cold_audit_key_is_architectural_w4_even_if_source_risk_is_w2(self):
        self.assertEqual(certification_key("OLLAMA", "auditor:1", "COLD_AUDIT", "W2", "S1", "B1").split("|")[3], "W4")

    def test_generated_certificate_is_runtime_source_version_and_coverage_bound(self):
        registry, key = self._registry_and_entry()
        self.assertTrue(is_certified_for_source(registry, key, source_units_sha256="4" * 64, source_unit_id="U1", provider="OLLAMA", model_alias="auditor:1", observed_version=DIGEST))
        self.assertFalse(is_certified_for_source(registry, key, source_units_sha256="4" * 64, source_unit_id="U1", provider="OLLAMA", model_alias="auditor:1", observed_version="sha256:" + "b" * 64))
        stale = copy.deepcopy(registry); del stale["certifications"][key]["cold_audit_dimension_coverage"]
        self.assertFalse(is_certified_for_source(stale, key, source_units_sha256="4" * 64, source_unit_id="U1", provider="OLLAMA", model_alias="auditor:1", observed_version=DIGEST))

    def test_tampered_dimension_coverage_invalidates_runtime_authority(self):
        registry, key = self._registry_and_entry(); tampered = copy.deepcopy(registry)
        tampered["certifications"][key]["cold_audit_dimension_coverage"]["per_dimension"][DIM_NEGATION]["accuracy"] = 0.0
        self.assertFalse(is_certified_for_source(tampered, key, source_units_sha256="4" * 64, source_unit_id="U1", provider="OLLAMA", model_alias="auditor:1", observed_version=DIGEST))

    def test_certificate_carries_semantic_fingerprint_for_lifecycle_freshness(self):
        registry, key = self._registry_and_entry(); self.assertEqual(registry["certifications"][key]["candidate_semantic_sha256"], "5" * 64)

...
class SourceUnit:
    ...
    def from_dict(cls, value: Dict[str, Any]) -> "SourceUnit":
        # Preserve dataclass defaults for fields absent from older source-unit
        # payloads. Passing value.get(...) for every field would silently turn
        # default dicts into None and weaken downstream invariants.
...
class AssertionCandidate:
    ...
    def from_dict(cls, value: Dict[str, Any]) -> "AssertionCandidate":
        # Keep backward compatibility with older run artifacts while preserving
        # current defaults for fields that did not exist yet.
    ...
    def validate_invariants(self) -> List[str]:
        ...
        for name, value in (
            ("stable_witness_sha256", self.stable_witness_sha256),

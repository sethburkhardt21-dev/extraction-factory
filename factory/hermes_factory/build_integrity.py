...
REQUIRED_EOL_POLICY_LINES = {"* -text"}



def _validate_certified_eol_policy(root: Path) -> None:
    root = Path(root).resolve()
    # Git EOL policy is a checkout portability control. Extracted/runtime-only
    # project roots without repository metadata are validated by their exact
    # manifest bytes and do not need a local .gitattributes file.
    if not (root.parent / ".git").exists():
        return
    policy = root / ".gitattributes"
    if not policy.is_file():
        raise RuntimeError("certified_eol_policy_missing")
    lines = {
        line.strip()
        for line in policy.read_text(encoding="utf-8").splitlines()
        if line.strip() and not line.lstrip().startswith("#")
    }
    missing = sorted(REQUIRED_EOL_POLICY_LINES - lines)
    if missing:
        raise RuntimeError("certified_eol_policy_incomplete:" + ",".join(missing))
...
def build_manifest(root: Path) -> Dict[str, Any]:
    root = Path(root)
    _validate_certified_eol_policy(root)
    files = []

...
def _provider_from_args(args, role: str):
    if args.mode == "offline-fixture":
        return DeterministicFixtureProvider(role=role)
    command = args.primary_command if role == "PRIMARY" else args.blind_command
    if not command:
        raise SystemExit(f"{role} provider command required for mode {args.mode}")
    prefix = "primary" if role == "PRIMARY" else "blind"
    return JSONCommandProvider(
        ...
        role=role,
        network_required=not args.local_provider,
